...
def make_data_loader(cfg, is_train=True, distributed=False, max_iter=None, start_iter=0):
    train_transform = build_transforms(cfg, is_train=is_train)             # 大小、尺寸、类型转变
    target_transform = build_target_transform(cfg) if is_train else None   # 数据转变：角点之类
    dataset_list = cfg.DATASETS.TRAIN if is_train else cfg.DATASETS.TEST   # 文件列表
